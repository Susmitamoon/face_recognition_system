...
class Student:
    def __init__(self,root):
        ...
        #bg image
        img3=Image.open(r"C:\Users\Susmita Saha\Desktop\Face_recognition Sytem\image\bag3.jpg")
        ...
        Left_frame.place(x=10,y=10,width=820,height=600)

        img_left=Image.open(r"C:\Users\Susmita Saha\Desktop\Face_recognition Sytem\image\man.jpg")
        ...
        dep_combo['values']=("Select Department","MCA", "IT","Civil","Mechanical")
        dep_combo.current(0)
        ...
        studentID_entry=ttk.Entry(class_Student_frame,textvariable=self.va_std_id,font=("times new roman",13,"bold"))
       ...
       # class_div_entry=ttk.Entry(class_Student_frame,textvariable=self.var_div,width=20,font=("times new roman",13,"bold"))
        ...
        div_combo.current(0)
        div_combo.grid(row=1,column=1,padx=10,pady=4,sticky=W)
        ...
        #radio button
        # Create a common variable for both radio buttons
        ...
        img_right=Image.open(r"C:\Users\Susmita Saha\Desktop\Face_recognition Sytem\image\man.jpg")
        ...
        search_frame=LabelFrame(Right_frame, bd=2,bg="white", relief=RIDGE,text="Student Information",font=("times new roman",12,"bold"))
        search_frame.place(x=5,y=135,width=710,height=70)

        search_label=Label(search_frame,text="Search By:",font=("times new roman",15,"bold"), bg="light blue", fg="dark blue")
    ...
    #----------------------------TABLE FRAME--------------------------------
        ...
        table_frame.place(x=5,y=200, width=710,height=300)
        ...
        self.student_table.column("photo", width=190, anchor=CENTER)
...
#-----------------FUNCTION DECLARATION------------------
    def add_data(self):
        if self.var_dep.get()=="Select Department" or self.var_std_name.get()=="" or self.va_std_id.get()=="":
            ...
            try:
                conn=mysql.connector.connect(host="localhost",username="root",password="WJ28@krhps",database="face_recognizer")
    ...
    # Check if required fields are filled
        if self.var_dep.get() == "Select Department" or self.var_std_name.get() == "" or self.va_std_id.get() == "":
            ...
            # Ask for confirmation to update
                ...
                if Update > 0:
                    print("Updating student with ID:", self.va_std_id.get())
                
                    # Connect to the database
                    conn = mysql.connector.connect(host="localhost", username="root", password="WJ28@krhps", database="face_recognizer")
                    my_cursor = conn.cursor()
                
                    # SQL query to update the student data based on the Student_id
                    my_cursor.execute("""
                        UPDATE student
                        SET Dep = %s,
                            course = %s,
                    ...
                    """, (
                        ...
                        self.var_teacher.get(),
                        self.var_radio1.get(),
                        self.va_std_id.get()  # Student ID to identify the record to update
                    ))
                ...
                # Optionally, refresh the data
                    ...
                    return  # If the user cancels the update
    ...
    def generate_dataset(self):
        if self.var_dep.get()=="Select Department" or self.var_std_name.get()=="" or self.va_std_id.get()=="":
            ...
            try:
                ...
                if result:
            # Student already exists - update PhotoSample later
                    is_existing = True
                else:
                    # Insert new student
                    insert_query = """
                        INSERT INTO student (
                            Dep, course, Year, Semester, Student_id, name, Division, Roll,
                    ...
                    my_cursor.execute(insert_query, (
                        ...
                        id, self.var_std_name.get(), self.var_div.get(), self.var_roll.get(),
                        self.var_gender.get(), self.var_dob.get(), self.var_email.get(), self.var_phone.get(),
                        self.var_address.get(), self.var_teacher.get(), "No"
                    ))
                    conn.commit()
                    is_existing = False
                ...
                # Update photo sample status in the database
                if is_existing:
                    my_cursor.execute("UPDATE student SET PhotoSample=%s WHERE Student_id=%s", ("Yes", id))
                    conn.commit()
                
                my_cursor.execute("UPDATE student SET PhotoSample=%s WHERE Student_id=%s",(str(self.var_radio1.get()), id))
                conn.commit()
